...
class FileBoard(AnyBoard):
                ...
                with open(self.file_path, 'r') as f:
                    if self._acquire_lock(f):
                        ...
                        if attempt < max_retries - 1:
                            ...
                            continue
                        raise RuntimeError(f"Could not acquire lock on {self.file_path} after {max_retries} attempts")
    ...
    def complete(self, task_id: str, assignee: str) -> bool:
        ...
        for story in stories:
            if story.task_id == task_id and story.assignee == assignee and not story.is_completed:
                story.complete()
                self._write_stories(stories)
                return True
    ...
    def get_pending_by_assignee(self, assignee: str) -> List[Story]:
        ...
        stories = self._read_stories()
        return [story for story in stories if story.assignee == assignee and not story.is_completed]
    
    def get_completed_by_assignee(self, assignee: str) -> List[Story]:
        """Get completed tasks assigned to a specific squad member."""
        stories = self._read_stories()
        return [story for story in stories if story.assignee == assignee and story.is_completed]
    ...
    def clear_completed(self, assignee: str) -> int:
        ...
        stories = [story for story in stories if not (story.assignee == assignee and story.is_completed)]
    ...
    def _assign_task_tool(self, assigner: str, assignee: str, task_name: str, description: str) -> Dict[str, Any]:
    ...
    def _create_assign_task_tool(self, agent_name: str) -> callable:
        ...
        def assign_task_to_agent(assignee: str, task_name: str, description: str) -> Dict[str, Any]:
            """
        ...
        assign_task_to_agent.__doc__ = f"Assign a new task to another agent. You are {agent_name}."
    ...
    def _create_complete_my_task_tool(self, agent_name: str) -> callable:
        ...
        complete_my_task.__doc__ = f"Complete a task that is assigned to you ({agent_name})."
            ...
            try:
                ...
                if new_tasks:
                    if len(new_tasks) == 1:
                        ...
                        return f"📋 You have {len(new_tasks)} new task(s) assigned to you"
                return ""
            except Exception as e:
            ...
            try:
                ...
                if new_completions:
                    if len(new_completions) == 1:
                        ...
                        return f"✅ Task completed by {task.assignee}: {task.description}"
                    else:
